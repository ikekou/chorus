...
> アイコンは `icons/` に同梱。2 種類あります — プレート版(`icon-*.png`、ストア・拡張一覧用)と
> 背景透明のツールバー版(`icon-tb-*.png`)。
> いずれも `python3 scripts/render-icons.py`(要 Pillow)で**透過付き**に生成します。
> 図柄や色はそのスクリプト内で編集して再実行してください。
