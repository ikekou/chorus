...
<img src="docs/images/popup.jpg" alt="Chorus のポップアップ:1つの入力欄、送信先(セット)セレクタ、モデルのチェックボックス" width="420">
...
> アイコンは `icons/`(`icon-16/32/48/128.png`)に同梱。角丸プレートで、**角の外側は透過**なので、
> ツールバー・拡張一覧・ストアのどこでも崩れません。
> `python3 scripts/render-icons.py`(要 Pillow)で生成。図柄や色はスクリプト内で編集して再実行してください。

## 注意点 / 既知の弱点

- **DOM セレクタは各社の UI 更新で壊れます。** 動かなくなったら `src/providers.js` のセレクタを修正してください。
- 入力欄は各社とも contenteditable(リッチエディタ)のため、`value` 代入では認識されません。本拡張では**合成 paste イベント**で挿入し(`execCommand('insertText')` と直接代入はフォールバック)、背面のウィンドウにも確実に入力できるようにしています。
